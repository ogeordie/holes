...
See the document `size matters and layout matters layout
algorithms.pdf` for details of the layout algorithms.
...
4.  Run the code. Confirm that `python --version` shows 3.13.x. Also ensure
    ...
    but this can be changed to create the data from scratch (although for
    LayoutExperiments.py this can take over 42 hours on a fast computer).
...
-   `Holes.py`: The code implementing the simulations.
...
-   `other data subdirectories`: contain the data generated by `LayoutExperiments.py`.
    ...
    (search for "go variables"). It may take over 42 hours to generate all data
...
Figures and Tables are output to the `graphs/sizematters` directory. By default `Experiments.py`
uses pre-generated data from the `data/sizematters` directory. If you wish to generate your own data
set `goData` to `True` in `Experiments.py`. It may take over an hour and 15 minutes on a fast
...
5:1, 20:1, etc, oriented both horizontally and vertically.
...
various directories (specified below). These are then used for plotting summary graphs.

#### Figures 3A, B, C, D, E: Layout Algorithm Success Curves
These graphs show the success of the various algorithms (on the y axis) plotted against the
...
#### Figures 4A, B, C, X [TODO]: Layout Algorithm Summary Experiments
Raw success data produced by `goSummaryData()` are stored in the `data/summary` directory
for various treasure areas.
...
**X** is `summary fieldsize100 siteArea100 maxholes6000.png`
...
#### Tables 2 and 3 Algorithm Summary Crossover Points
These use the `summary` files in the `data/summary` directories (as for Figures 4A, B, C, X)
produced by `goSummaryGraphs()` to display the treasure elongation ratios
at which the performance of various layout algorithms "crosses over" (starts to perform
worse than) the performance of the `Halton` layout algorithm as the elongation ratio
increases (in both the horizontal and vertical directions). See the paper and 
`generateCrossoverTableAndGraph()` in `LayoutExperiment.py` for more information.
`goCrossoverTables()` generates the tables in the `graphs/summary` directory.

**2** is `horizontal crossover fieldsize100 table.png`   
**3** is `vertical crossover fieldsize100 table.png`

#### Figures 5A, B, C: Low Intensity Test Pit Summary Curves
These are summary graphs but only over 1 to 25 holes.
They use the raw success data in the `data/summary` directory (as for Figures 4A, B, C, X).
...
#### Figures 5A, B: Real-World Site Success Curves
...
#### Tables 4 and 5: 
These use the same data as in Figures 5A and B, and shows the number of holes needed
to achieve three success thresholds (80%, 95%, 100%) for the various layout algorithms,
for both horizontally and vertically oriented real world site shape.
The tables are generated by `goRealWorldHolesForSuccessRatesTables()` in the `graphs/realworld`
directory.

**4** is `holes for success rates table horizontalpolygon fieldsize100.png`  
**5** is `holes for success rates table verticalpolygon fieldsize100.png`
